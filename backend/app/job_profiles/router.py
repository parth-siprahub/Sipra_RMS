...
async def create_job_profile(
    ...
    dup = client.table("job_profiles").select("id").eq("role_name", payload.role_name).execute()
    if dup.data:
        raise HTTPException(status.HTTP_409_CONFLICT, f"Job profile '{payload.role_name}' already exists")
    result = client.table("job_profiles").insert(payload.model_dump(exclude_none=True)).execute()
...
async def update_job_profile(
    ...
    result = client.table("job_profiles").update(data).eq("id", profile_id).execute()
    if not result.data:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Job profile not found")
    return result.data[0]
...
async def delete_job_profile(
    ...
    linked = client.table("resource_requests").select("id").eq("job_profile_id", profile_id).limit(1).execute()
    if linked.data:
        raise HTTPException(status.HTTP_409_CONFLICT, "Cannot delete: linked to resource requests")
    client.table("job_profiles").delete().eq("id", profile_id).execute()
